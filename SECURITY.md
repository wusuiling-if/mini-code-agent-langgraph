...
- `mca sandbox probe` uses disposable data without contacting a model provider or target repository. Native backends must read an exact host sentinel and deny its mutation; Docker must not see the host sentinel and must separately deny a write to its read-only root. `bwrap` and Docker must hide the controlled and known host Unix sockets, while `sandbox-exec` may expose them only when connection is denied. Network isolation requires the process to be unable to obtain or use an outbound route to a TEST-NET address (tested by UDP `connect` without sending a packet) and denial of a controlled loopback TCP connection. Negative executor returns, timeouts, exceptions, missing preconditions, and an unavailable verified `/var/tmp` base fail the probe. It rejects `--sandbox none`; a passing probe is evidence for these checks only, not a general proof of safe execution.
