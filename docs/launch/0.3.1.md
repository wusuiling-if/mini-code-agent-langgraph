...
Technical discussion areas include the verification boundary, trajectory format,
and Undo threat model.
...
2. Hacker News, only after the repository is runnable: do not submit 0.3.1 as a routine version announcement. A Show HN submission may present the runnable project as a whole only if the maintainer personally built it, it is non-trivial and easy to try, and the maintainer is available to discuss it. The title must start with `Show HN`, link to the runnable repository, and explain why it was built. Do not paste generated or AI-edited copy, solicit votes/comments, ask friends to engage, delete-and-repost, or frame a routine patch release as the whole reason to submit.
3. V2EX `分享创造`, only as a maintainer-authored Chinese post: disclose that it is your project, describe concrete implementation details and limitations, and respond substantively to organic questions. V2EX explicitly says not to send AI-generated content, so do not paste or lightly edit this draft; write the post yourself from verified project facts. Do not duplicate it across nodes or use zero-information replies to bump it.
